...
class ValidadorProventosDeAposentadoria:
    ...
    def predict(self):
        ...
        resultados = {
            'proventos_de_aposentadoria': { 'predict': False, 'explain': "Registro dos proventos de aposentadoria."},
        }
        ...
        # Vamos Manter todas as linhas que contêm 'aposent' na coluna e, além disso,
        ...
        try:
            ...
            mask = self.df[column_to_filter].str.contains('aposent', regex=True, na=False, case=False)
        ...
        resultados['proventos_de_aposentadoria']['explain'] += self.explain(isvalid_nome, 'nome')

        # Cargo
        isvalid_cargo, _ = self.predict_cargo()
        resultados['proventos_de_aposentadoria']['explain'] += self.explain(isvalid_cargo, 'cargo')

        # Remuneração
        isvalid_remuneracao, _ = self.predict_remuneracao()
        resultados['proventos_de_aposentadoria']['explain'] += self.explain(isvalid_remuneracao, 'remuneração')

        # Abate teto
        isvalid_abate_teto, _ = self.predict_abate_teto()
        resultados['proventos_de_aposentadoria']['explain'] += self.explain(isvalid_abate_teto, 'abate teto')

        # Remuneração retirando o abate teto
        isvalid_retirando_o_abate_teto, _ = self.predict_remuneracao_retirando_o_abate_teto()
        resultados['proventos_de_aposentadoria']['explain'] += self.explain(isvalid_retirando_o_abate_teto, 'remuneração retirando o abate teto')

        # Tipo de vínculo
        isvalid_tipo_de_vinculo, _ = self.predict_tipo_de_vinculo()
        resultados['proventos_de_aposentadoria']['explain'] += self.explain(isvalid_tipo_de_vinculo, 'tipo de vínculo')
        ...
        if any([isvalid_nome, 
            ...
            resultados['proventos_de_aposentadoria']['predict'] = True
